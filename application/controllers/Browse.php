...
class Browse extends CI_Controller {
	...
	function __construct()
	{
		...
		if($called_function == 'search' || $called_function == 'process_list' || $called_function == 'home'  ||
		  		$called_function == 'live' || $called_function == 'movie' || $called_function == 'mylist' || $called_function == 'series' || $called_function == 'playmovie' || $called_function == 'playseries') {
	...
	function live($genre_id = '', $offset = '')
	{
		$page_data['page_name']		=	'live';
		$page_data['page_title']	=	'Watch live';
		$page_data['genre_id']	=	$genre_id;

		// pagination configuration
		$url = base_url() . 'index.php?browse/live/' . $genre_id;
        $per_page = 20;
		$this->db->where('genre_id' , $genre_id);
        $total_result = $this->db->count_all_results('live');
        $config = $this->crud_model->paginate($url, $total_result, $per_page, 4);
        $this->pagination->initialize($config);

        $page_data['live'] = $this->crud_model->get_live($genre_id , $per_page, $this->uri->segment(4));
	...
	function playlive($live_id = '')
	{
		$page_data['page_name']		=	'playlive';
		$page_data['page_title']	=	'Watch live';
	...
	function channel($genre_id = '', $offset = '')
	{
		$page_data['page_name']		=	'channel';
		$page_data['page_title']	=	'Watch channel';
		$page_data['genre_id']	=	$genre_id;

		// pagination configuration
		$url = base_url() . 'index.php?browse/channel/' . $genre_id;
        $per_page = 20;
		$this->db->where('genre_id' , $genre_id);
        $total_result = $this->db->count_all_results('channel');
        $config = $this->crud_model->paginate($url, $total_result, $per_page, 4);
        $this->pagination->initialize($config);

        $page_data['channel'] = $this->crud_model->get_channel($genre_id , $per_page, $this->uri->segment(4));
		$page_data['total_result']	=	$total_result;
	...
	{
		$page_data['page_name']		=	'playchannel';
		$page_data['page_title']	=	'Watch channel';
